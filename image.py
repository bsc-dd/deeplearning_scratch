...
class Image(object):
    ...
    def get_image(filename, prefix='/', spacing=None, maxintensity=None):
        ...
        if isstring(filename):
            ...
            elif filename.endswith('.hecuba'):
                return HecubaImage(filename, spacing=spacing, maxintensity=maxintensity)
            else:
    ...
    def crop_from_array(arr, bbox, arr_spacing=1., wanted_spacing=1., interpolation=CUBIC, dtype=None, blocksize=8000):
        ...
        output_size = tuple(
            [int(math.floor(s * arr_spacing / wanted_spacing)) for s in bbox.get_hw(round_values=False)])
        if len(arr.shape) > 2:
        ...
        if not (blocksize * arr_spacing / wanted_spacing).is_integer():
            log.warning(
                "blocksize * scale is not a natural number:{}. This may lead to problems in blockwise resizing.".format(
                    blocksize * arr_spacing / wanted_spacing))
        for box in blocks(bbox, (blocksize, blocksize)):
            ...
            output_size = tuple(
                [int(math.floor(s * (arr_spacing / wanted_spacing))) for s in box.get_hw(round_values=False)])
...
class ImgLevels(StorageDict):
    '''
    @ClassType <<id:int>,value:numpy.ndarray>
    '''
    pass


class HecubaImage(Image):
    """Represents an image stored with Hecuba.

    Supports creation of pyramids, extraction of crops and saving as tiff or hdf5 image.
    """

    def __init__(self, filename, spacing=1., maxintensity=None):
        """ Initialize MemoryImage.
        ...
        Args:
            ...
            maxintensity (int): maxintensity of pixel values in the image.
        """
        levels = ImgLevels(filename)
        scales = [1.]
        if maxintensity is None:
            maxintensity = dtype_limits(levels[0].dtype)[1]

        size = levels[0].shape[:2]
        super(HecubaImage, self).__init__(spacing, size, scales, maxintensity)
        self._levels = levels

    def __repr__(self):
        return 'HecubaImage(arr={}, spacing={}, maxintensity={})'.format(
            self._levels[0], self.spacing, self.maxintensity)
